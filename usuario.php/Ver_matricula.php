...
$sql = "SELECT m.id, a.ano, c.nome_curso, pr.nome AS periodo, s.num_sala, t.nome_turma, cl.nome_classe
        FROM matricula m
...
<!-- Conteúdo principal -->
<div class="container">
<button class="btn btn-success print-btn" onclick="window.print()">Imprimir Histórico</button>
    <h2 class="mb-4">Dados da Matrícula</h2>

    <?php
    if ($result && $result->num_rows > 0) {
        $row = $result->fetch_assoc();
        echo "<h4>Você está matriculado no seguinte curso:</h4>";
        echo "<table class='table table-bordered table-striped'>";
        echo "<tr><th>Ano Letivo</th><td>{$row['ano']}</td></tr>";
        echo "<tr><th>Curso</th><td>{$row['nome_curso']}</td></tr>";
        echo "<tr><th>Período</th><td>{$row['periodo']}</td></tr>";
        echo "<tr><th>Sala</th><td>{$row['num_sala']}</td></tr>";
        echo "<tr><th>Turma</th><td>{$row['nome_turma']}</td></tr>";
        echo "<tr><th>Classe</th><td>{$row['nome_classe']}</td></tr>";
        echo "</table>";
        
        // Adicionar link para o histórico de classes
        echo "<div class='mt-4'>";
        echo "</a>";
        echo "</div>";
    } else {
        echo "<div class='alert alert-warning'>Você não está matriculado em nenhum curso no momento.</div>";
    }
    ?>
